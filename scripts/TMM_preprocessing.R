...
tmm_preprocessing <- function(file, referenceSample=NULL){
  ...
  if(is.null(referenceSample)){
    ...
    data_list_tmm = calcNormFactors.modified(data)
    
    if(ncol(data)!=length(data_list_tmm$f)){
      stop("Mismatch between number of samples and number of scaling factors.")
    } else {
      #apply scaling factor
      data_scaled = sweep(data, 2, as.array(data_list_tmm$f), `*`)
      #log cpm transformation (EdgeR preprocessing)
      data_cpm = log(sapply(seq(ncol(data)), function(c){
        (data[,c]/sum(data_scaled[,c]))*1e6}))
      colnames(data_cpm) <- colnames(data_scaled)
    }

    write.csv(t(data_cpm), file.path(args$output_directory, paste0(run_id, '_train-TMM_none.csv')))

    ref = data[,data_list_tmm$ref]
    
    return(list(ref=ref,
                 factor_scaling=data_list_tmm$factor_scaling,
                 feature_names_in=rownames(data_cpm)))
    ...
    if(ncol(data_f)!=length(data_list_tmm$f)){stop("Mismatch between number of samples and number of scaling factors.")
      ...
      write.csv(t(data_cpm_plus[,-1]), file=file.path(args$output_directory, paste0(run_id, '_', tmmname, '-TMM_none.csv')))
    }
  }
}
...
if(args$fit == 'True'){
    train_params = tmm_preprocessing(args$train_data)
    suppress = sapply(args$test_data, function(t){
	       tmm_preprocessing(t, referenceSample=train_params)})

	# Here args$params references a directory for output
 	saveRDS(train_params, file = file.path(args$params, 'TMM_parameters.txt'), ascii = TRUE, compress = FALSE)
#     write_json(train_params, file.path(args$params, 'TMM_parameters.json'))
}
